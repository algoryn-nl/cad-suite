...
Defer CAD handoff details to the CAD skill: read `${CLAUDE_PLUGIN_ROOT}/skills/cad/SKILL.md`, then only load its `references/rendering-and-explorer.md` when a URDF needs a CAD Explorer link. If the local CAD skill is unavailable, use [cad-skill](https://github.com/earthtojake/cad-skill) as the fallback reference. Keep URDF-specific generation and `explorer_metadata` contracts in this skill.
